...
void	ft_sort_three(t_dlst *stack_a, int *ops)
{
	...
	if (nbrs[0] <= nbrs[1] && nbrs[1] <= nbrs[2])  // works  (1 2 3)
		return ;
	else if (nbrs[1] <= nbrs[0] && nbrs[0] <= nbrs[2]) // works (2 1 3)
		ft_sa(stack_a, ops);
	else if (nbrs[2] <= nbrs[0] && nbrs[0] <= nbrs[1])  // works (2 3 1)
		ft_rra(stack_a, ops);
	else if (nbrs[1] <= nbrs[2] && nbrs[2] <= nbrs[0])  // works (2 1 3)
		ft_ra(stack_a, ops);
	else if (nbrs[0] <= nbrs[2] && nbrs[2] <= nbrs[1])  // works (1 3 2)
	{
		ft_sa(stack_a, ops);
		ft_ra(stack_a, ops);
	}
	else if (nbrs[2] <= nbrs[1] && nbrs[1] <= nbrs[0]) // works (3 2 1)
	{
		ft_sa(stack_a, ops);
		ft_rra(stack_a, ops);
	}
